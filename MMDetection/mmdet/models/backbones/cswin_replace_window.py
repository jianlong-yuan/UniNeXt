...
def local_group(x, H, W, ws, ds):
    ...
    if H % ws != 0 or W % ws != 0:
        ...
        x = x.view(B, N, C)
    x_row, x_column = x[:, :, :C//2], x[:, :, C//2:C]
    Gh = H//ws
    Gw = W//ws
    x_row = x_row.view(B, Gh, ws, 1, W, C//2).permute(0, 1, 3, 2, 4, 5).contiguous().view(B*Gh, ws*W, C//2)
    x_column = x_column.view(B, 1, H, Gw, ws, C//2).permute(0, 1, 3, 2, 4, 5).contiguous().view(B*Gw, H*ws, C//2)
    # x = x.view(B, Gh, ws, Gw, ws, C).permute(0, 1, 3, 2, 4, 5).contiguous().view(B*Gh*Gw, ws*ws, C)
    
    return x_row, x_column, H, W, pad_right, pad_bottom, pad_opt
...
def img2group(x, H, W, ws, ds, num_head):
    ...
    x_row, x_column, H, W, pad_right, pad_bottom, pad_opt = local_group(x, H, W, ws, ds)
    B_row, N_row, C =x_row.shape
    x_row = x_row.view(B_row, N_row, num_head, C//num_head).permute(0, 2, 1, 3).contiguous()
    B_column, N_column, C =x_column.shape
    x_column = x_column.view(B_column, N_column, num_head, C//num_head).permute(0, 2, 1, 3).contiguous()

    return x_row, x_column, H, W, pad_right, pad_bottom, pad_opt


def group2image(x_row, x_column, H, W, pad_right, pad_bottom, pad_opt, ws):
    # Input x: (BG G) Head n C
    # Output x: B N C

    BG, Head, n, C = x_row.shape
    Gh, Gw, hc = H//ws, W//ws, Head*C
    B = BG//Gh
    x_row = x_row.view(B, Gh, Head, ws, W, C).permute(0, 1, 3, 4, 2, 5).contiguous().view(B, -1, hc)
    x_column = x_column.view(B, Gw, Head, H, ws, C).permute(0, 3, 1, 4, 2, 5).contiguous().view(B, -1, hc)
    
    if pad_opt:
        x_row = x_row.view(B, H, W, hc)
        x_row = x_row[:, :(H - pad_bottom), :(W - pad_right), :].contiguous()  # remove
        x_row = x_row.view(B, -1, hc)
        
        x_column = x_column.view(B, H, W, hc)
        x_column = x_column[:, :(H - pad_bottom), :(W - pad_right), :].contiguous()  # remove
        x_column = x_column.view(B, -1, hc)

    x = torch.cat([x_row, x_column], dim=2)
    return x
...
class DilatedAttention(nn.Module):
    ...
    def forward(self, qkv, H, W):
        ...
        if self.last_stage:
            q = q.view(B, N, self.num_heads, vc//self.num_heads).permute(0, 2, 1, 3).contiguous()
            k = k.view(B, N, self.num_heads, vc//self.num_heads).permute(0, 2, 1, 3).contiguous()
            v = v.view(B, N, self.num_heads, vc//self.num_heads).permute(0, 2, 1, 3).contiguous()
            q = q * self.scale
            ...
            x = attn @ v
            x = x.permute(0, 2, 1, 3).contiguous().view(B, N, vc)
        else:
            # B,N,C -->BG,H,N,C
            q_row, q_column, H_new, W_new, pad_right, pad_bottom, pad_opt = img2group(q, H, W, self.ws, self.ds, self.num_heads)
            k_row, k_column, _, _, _, _, _ = img2group(k, H, W, self.ws, self.ds, self.num_heads)
            v_row, v_column, _, _, _, _, _ = img2group(v, H, W, self.ws, self.ds, self.num_heads)
            q_row = q_row * self.scale
            # B head N C @ B head C N --> B head N N
            attn_row = (q_row @ k_row.transpose(-2, -1))
            attn_row = nn.functional.softmax(attn_row, dim=-1, dtype=attn_row.dtype)
            attn_row = self.attn_drop(attn_row)
            x_row = attn_row @ v_row
            
            attn_column = (q_column @ k_column.transpose(-2, -1))
            attn_column = nn.functional.softmax(attn_column, dim=-1, dtype=attn_row.dtype)
            attn_column = self.attn_drop(attn_column)
            x_column = attn_column @ v_column
            
            # x: BHnC -> BNC
            x = group2image(x_row, x_column, H_new, W_new, pad_right, pad_bottom, pad_opt, self.ws)
...
class CswinReplaceWindow(BaseModule):
    ...
    def __init__(self, 
                 ...
                 use_chk=False,
                 init_cfg=None):
        super(CswinReplaceWindow, self).__init__(init_cfg=init_cfg)
