...
def main():
    ...
    if args.num_gpu > 1:
        if use_amp == 'apex':
            ...
            use_amp = None
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=list(range(args.num_gpu)), broadcast_buffers=False).cuda()
    ...
    if args.distributed:
        ...
        if has_apex and use_amp != 'native':
            ...
            if args.resume:
                ...
                loss_scaler.load_state_dict(state['amp'])

            # model = ApexDDP(model, delay_allreduce=True)
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], broadcast_buffers=False).cuda()
        else:
            if args.local_rank == 0:
                _logger.info("Using native Torch DistributedDataParallel.")
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], broadcast_buffers=False).cuda()  # can use device str in Torch >= 1.1
