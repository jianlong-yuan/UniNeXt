...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False,pretrained_cfg=None, bn_tf=None):
        super().__init__()
...
@register_model
def cswin(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[2,2,18,2],
...
def cswin_small(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2,2,18,2],
        split_size=[1,2,7,7], num_heads=[3,6,12,24], mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model

@register_model
def cswin_base(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=128, depth=[2,2,18,2],
